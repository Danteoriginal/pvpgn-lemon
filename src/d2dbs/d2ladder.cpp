...
extern int d2ladder_update(t_d2ladder_info * pcharladderinfo)
{
	...
	if (!expansion && classs> D2CHAR_CLASS_MAX ) return -1; 
	if (expansion && classs> D2CHAR_EXP_CLASS_MAX ) return -1; 
	...
	ladder_class_type=ladder_overall_type + classs+1; 
...
int d2ladder_readladder(void)
{
	...
	for(laddertype=0;laddertype<d2ladder_maxtype;laddertype++) {
		...
		for (i=0; i< number; i++) {
			...
			temp.classs=bn_byte_get(ldata[i].classs);
...
const char * get_prefix(int type, int status, int classs)
{
  int  difficulty;
  ...
  return prefix[difficulty][type][sex[classs]];
}
  ...
  for(type=0; type <d2ladder_maxtype; type++) {
    ...
    for(i=0; i<d2ladder->len; i++)
	  ...
	  fprintf(ladderstrm,"\t\t<experience>%u</experience>\n\t\t<classs>%s</classs>\n",
		             ldata[i].experience,charclass[ldata[i].classs+1]);
	  fprintf(ladderstrm,"\t\t<prefix>%s</prefix>\n",
		             get_prefix(overalltype,ldata[i].status,ldata[i].classs+1));
...
*/
	...
	for(i=0;i<d2ladder_maxtype;i++) {
		...
		for (j=0; j< number; j++) {
			...
			bn_byte_set(&ldata[j].classs, d2ladder->info[j].classs);
...
int d2ladder_print(FILE *ladderstrm)
{
	...
	for(type=0; type <d2ladder_maxtype; type++) {
		...
		fprintf(ladderstrm,"************************************************************************\n");
		fprintf(ladderstrm,"No    character name    level      exp       status   title   classs     \n");
		for(i=0; i<d2ladder->len; i++)
		{
