...
extern int attrgroup_flush(t_attrgroup *attrgroup, int flags)
{
	...
	BEGIN(_attr,attrgroup->list)
	{
		attr=*_attr;
        attr_destroy(attr);
    }
	attrgroup->list.clear();
...
static t_attr *attrgroup_find_attr(t_attrgroup *attrgroup, const char **pkey, int escape)
{
	...
	string findKey(*pkey);
	boost::algorithm::to_lower(findKey);

	BOOST_AUTO(it_res,attrgroup->list.get<tag_name>().find(findKey));
	BOOST_AUTO(it_err,attrgroup->list.get<tag_name>().end());
    ...
    {	/* no key found in cached list */
        attr = (t_attr*)storage->read_attr(attrgroup->storage, *pkey);
        if (attr) attrgroup->list.insert(attr);
    }

//	if (escape)xfree((void*)*pkey);
...
extern int attrgroup_set_attr(t_attrgroup *attrgroup, const char *key, const char *val)
{
    ...
    if (attr) {
        ...
        /* new_ value for existent key, replace the old one */
        attr_set_val(attr, val);
    } else {	/* unknown key so add new_ attr */
        attr = attr_create(newkey, val);
		attrgroup->list.insert(attr);
