...
#ifdef CONNECTION_INTERNAL_ACCESS
#define MAKE_SHARED_FUNCTOR(mother,type,member,name) \
struct name\
{\
  typedef type result_type; \
  const result_type& operator()(const mother r)const \
  {\
    return r->member;\
  }\
}
...
MAKE_SHARED_FUNCTOR(t_connection*,t_clienttag,protocol.client.clienttag,t_connection_clienttag);
MAKE_SHARED_FUNCTOR(t_connection*,unsigned int ,protocol.sessionkey,t_connection_session);
MAKE_SHARED_FUNCTOR(t_connection*,int,socket.tcp_sock,t_connection_tcp_sock);
MAKE_SHARED_FUNCTOR(t_connection*,unsigned int ,socket.tcp_addr,t_connection_tcp_addr);
...
<
	...
	indexed_by
	<
		hashed_unique<tag<tag_session>,t_connection_session>,
		hashed_unique<tag<tag_tcp_sock>,t_connection_tcp_sock>,
		hashed_non_unique<tag<tag_tcp_addr>,t_connection_tcp_addr>,
		hashed_non_unique<tag<tag_clienttag>,t_connection_clienttag>
	>
