...
static int file_write_attrs(t_storage_info *, const attrs_type&);
...
static int file_write_attrs(t_storage_info * info, const attrs_type&attributes)
{
    char *tempname;
...
static int file_write_clan(void *data)
{
	...
	if ((member->status == CLAN_NEW) && (time(NULL) - member->join_time > prefs_get_clan_newer_time() * 3600))
	    member->status = CLAN_PEON;
	fprintf(fp, "%i,%c,%u\n", account_get_uid((const t_account *)member->memberacc), member->status + '0', (unsigned) member->join_time);
    }
