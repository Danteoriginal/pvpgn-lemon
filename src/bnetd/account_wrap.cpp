...
extern int account_get_auth_admin(t_account * account, char const * channelname)
{
    ...
    if (!(temp = account_get_strattr(account,"BNET\\auth\\admin")))
        return -1;
    if(channelname)
    {
        return !strcasecmp(temp,channelname);
    }
    switch (str_get_bool(temp))
    {
    case 1:
...
extern int account_set_auth_admin(t_account * account, char const * channelname, int val)
{
    if(!channelname)
	{
		if(val)return account_set_strattr(account, "BNET\\auth\\admin", "1");
		return account_set_strattr(account, "BNET\\auth\\admin", "0");
	}
    if(str_get_bool(channelname)==1)return 0;
    if(val)return account_set_strattr(account, "BNET\\auth\\admin", channelname);
    else return account_set_strattr(account, "BNET\\auth\\admin", "0");
}
...
extern int account_get_auth_operator(t_account * account, char const * channelname)
{
    ...
    if (!(temp = account_get_strattr(account,"BNET\\auth\\operator")))
        return -1;
    if(channelname)
    {
        return !strcasecmp(temp,channelname);
...
extern int account_set_auth_operator(t_account * account, char const * channelname, int val)
{
    if(!channelname)
	{
	    if(val)return account_set_strattr(account, "BNET\\auth\\operator", "1");
	    return account_set_strattr(account, "BNET\\auth\\operator", "0");
	}
    if(str_get_bool(channelname)==1)return 0;
    if(val)return account_set_strattr(account, "BNET\\auth\\operator", channelname);
    else return account_set_strattr(account, "BNET\\auth\\operator", "0");
}
...
extern int account_get_auth_voice(t_account * account, char const * channelname)
{
    ...
    if (!(temp = account_get_strattr(account,"BNET\\auth\\voice")))
        return -1;
    if(channelname)
    {
        return !strcasecmp(temp,channelname);
...
extern int account_set_auth_voice(t_account * account, char const * channelname, int val)
{
    ...
    if(str_get_bool(channelname)==1)return 0;
	if(val)return account_set_strattr(account, "BNET\\auth\\voice", channelname);
    else return account_set_strattr(account, "BNET\\auth\\voice", "0");
}
...
extern int account_set_normal_class(t_account * account, t_clienttag clienttag, unsigned int classs)
{
    char key[256];
    char clienttag_str[5];
    ...
    sprintf(key,"Record\\%s\\0\\class",tag_uint_to_str(clienttag_str,clienttag));
    return account_set_numattr(account,key,classs);
}
...
extern int account_remove_friend2( t_account * account, const char * friend_)
{
    t_list *flist;
    t_friend *fr;
    unsigned i, uid;
    ...
    if (friend_ == NULL) {
        eventlog(eventlog_level_error, __FUNCTION__, "got NULL friend_ username");
        return -1;
    }
    ...
    if ((fr = friendlist_find_username(flist, friend_)) == NULL) return -2;
...
{
    if(account_get_auth_admin(account,NULL)==1)return 4;
    if(account_get_auth_operator(account,NULL)==1)return 3;
    if(channel)if(account_get_auth_admin(account,channel)==1)return 2;
    if(channel)if(account_get_auth_operator(account,channel)==1)return 1;
    return 0;
}
