...
/*    {
        ...
        psock_setsockopt(csocket,PSOCK_SOL_SOCKET,SO_RCVBUF,&val,(psock_t_socklen)sizeof(val));
        psock_setsockopt(csocket,PSOCK_SOL_SOCKET,SO_SNDBUF,&val,(psock_t_socklen)sizeof(val));
        /* not a fatal error /
    }*/
...
static int sd_tcpinput(t_connection * c)
{
    ...
    case 1: /* done reading */
        ...
        case conn_class_telnet:
            ...
            {
                char const * const temp=(const char*)packet_get_raw_data_const(packet,0);
...
/*    if (_setup_add_addrs(&laddrs, prefs_get_bnetdserv_addrs(),INADDR_ANY,6113,laddr_type_bnet))
    {
        eventlog(eventlog_level_error,__FUNCTION__,"could not create %s server address list from \"%s\"",laddr_type_get_str(laddr_type_bnet),prefs_get_bnetdserv_addrs());
        return -1;
    }*/
