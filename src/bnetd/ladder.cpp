...
/*        if(ladder->cnt>1500)
        {
            if(level > ladder->last->level)
            {
                xfree(ladder_entry);
                return 0;
            }
        }*/
        if ((ladder->first->level - level) >= (level - ladder->last->level))
...
extern int war3_ladder_update(t_ladder *ladder, int uid, int xp, int level, t_account *account, unsigned int teamcount)
{
    ...
    search = ladder->first;
    while (search && (search->uid != uid)) { search = search->prev;}
    if (search != NULL)
    {
        search->xp += xp;
        search->level = level;
    ...
    {
        war3_ladder_add(ladder,uid,xp,level,account,-1,CLIENTTAG_BROODWARS_UINT);
    }
    return 0;*/
}
