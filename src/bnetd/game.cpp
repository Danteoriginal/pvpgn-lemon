...
static int game_evaluate_results(t_game * game)
{
	...
	if(game->count==2)
	{
		if((game->results[0]==game_result_none || game->results[0]==game_result_loss|| game->results[0]==game_result_draw)
			&&game->results[1]==game_result_disconnect)
			game->results[0]=game_result_win;
		if((game->results[1]==game_result_none || game->results[1]==game_result_loss|| game->results[1]==game_result_draw)
			&&game->results[0]==game_result_disconnect)
			game->results[1]=game_result_win;
		if(game->results[0]==game_result_win && game->results[1]==game_result_win)
		{
			game->results[0]=game->results[1]=game_result_draw;
			game->bad=1;
		}
	}
    return 0;
    ...
    else
    {
        ...
        for (i=0; i<game->count; i++)
        {
            ...
            if (game->results[i]!=game_result_none)
            {
                if(i!=realcount)
                {
                    pp=(void*)game->players[realcount];
                    game->players[realcount]       = game->players[i];
                    game->players[i]=(t_account*)pp;

                    kk=game->results[realcount];
                    game->results[realcount]       = game->results[i];
                    game->results[i]=(t_game_result)kk;

                    pp=(void*)game->report_heads[realcount];
                    game->report_heads[realcount]       = game->report_heads[i];
                    game->report_heads[i]=(const char*)pp;

                    pp=(void*)game->report_bodies[realcount];
                    game->report_bodies[realcount]       = game->report_bodies[i];
                    game->report_bodies[i]=(const char*)pp;

                    pp=(void*)game->reported_results[realcount];
                    game->reported_results[realcount]       = game->reported_results[i];
                    game->reported_results[i]=(t_game_result*)pp;
				}
    ...
    if (realcount>=1 && !game->bad)
    {
           ...
           )
        {
            ...
            if(realcount==2)
            {
                ...
                {
                    tt=account_get_numattr(game->players[i],"BNET\\acct\\durtime");
                    if(tt<720){ttt=0;}
                }
                ...
                if(ttt)
                    for (i=0; i<realcount; i++)
                    {
                        ...
                        uid=account_get_uid(game->players[1-i]);
                        if(uid<1)continue;
                        j=uid;
                        for(k=0;k<5;k++)
                        {
                        ...
                        ustr[5]=0;
                        in=account_get_strattr(game->players[i],"BNET\\played");
                        if(!in)
                        {
                            account_set_strattr(game->players[i],"BNET\\played",ustr);
                            continue;
                        }
                        for(k=0;k<240;k+=5)
                        {
                        ...
                        if(strlen(in)>235)strcpy(out,in+5);
                        else strcpy(out,in);
                        strcpy(out+strlen(out)-(strlen(out)%5),ustr);
                        account_set_strattr(game->players[i],"BNET\\played",out);
                    }

                a=account_get_ladder_rating(game->players[0],CLIENTTAG_BROODWARS_UINT,1);
                b=account_get_ladder_rating(game->players[1],CLIENTTAG_BROODWARS_UINT,1);
                if(a>b)c=a-b;else c=b-a;
                if(a>=1100&&b>=1100&&c<=100)ttt=1;
                if(a>0&&b>=1000&&c>200)ttt=0;

//                localtime(&now);
//                if(((unsigned long int)difftime(now,game->start_time))/60<3)ttt=0;
            }
            else ttt=0;
        }
        if(ttt)
    ...
    if (!game_is_ladder(game) && prefs_get_report_all_games()!=1)
    {
    ...
    if(!ttt || !ladder_info)
    {
        eventlog(eventlog_level_debug,__FUNCTION__,"not reporting normal games2");
        return 0;
    }
            ...
            game_option_get_str(game->option));
    {
        ...
        fprintf(fp,"created=\"%s %d\" ",timetemp,mktime(gametime));

        if (!(gametime = localtime(&game->start_time)))
            strcpy(timetemp,"?");
        else
            strftime(timetemp,sizeof(timetemp),GAME_TIME_FORMAT,gametime);
        fprintf(fp,"started=\"%s %d\" ",timetemp,mktime(gametime));

        if (!(gametime = localtime(&now)))
            strcpy(timetemp,"?");
        else
            strftime(timetemp,sizeof(timetemp),GAME_TIME_FORMAT,gametime);
        fprintf(fp,"ended=\"%s %d\" ",timetemp,mktime(gametime));
    }
    {
        char const * mapname;
            ...
            game->clienttag==CLIENTTAG_WARCIIBNE_UINT)
    {
        for (i=0; i<realcount; i++)
		{
			fprintf(fp,"[Player normal record %d]\n",i+1);
            ...
            game->clienttag==CLIENTTAG_WARCIIBNE_UINT)
    {
        ...
        for (i=0; i<realcount; i++)
		{
			fprintf(fp,"[Player ladder record %d]\n",i+1);
...
extern int _isTeamgame(const t_game *game,t_connection *c = 0)
{
	int i,cntMax=0,j;
	int gameCnt=game_get_count(game);

	if(gameCnt<=0)
		return 0;

	vector<const char *> stat;
	stat.resize(gameCnt);
	...
	REP(i,gameCnt-1)
 	...
 	REP(i,gameCnt-1)
		if (game_get_player_conn(game,i) != NULL)
		{
			stat[i]=account_get_strattr(conn_get_account(game_get_player_conn(game,i)),"BNET\\team");
			cntMax++;
		}
 	...
 	REP(i,gameCnt-1)
	{
		...
		REPTO(j,i+1,gameCnt-1)
		{
			if(!stat[j])continue;
			if(stat[j][0]==0)continue;
		...
		{
			char msgtemp[80];
			sprintf(msgtemp,"'%s'팀 : %d 명",stat[i],cnt);
			message_send_text(c,message_type_info,c,msgtemp);
		}
		if(cnt!=cntMax/2 && cnt!=cntMax)
			return 1;
	}
	return 0;
}
...
extern void game_set_status(t_game * game, t_game_status status)
{
    ...
    if (status==game_status_started && game->start_time==(time_t)0)
	{
		...
		for (i=0; i<game->count; i++)
		{
			if (game->connections[i] == NULL)continue;
			j++;
		}
		if (j>2 && game->type==game_type_topvbot)
		{
			if(_isTeamgame(game))
			{
//				game_send_mes_t(game,"[ 팀 대전으로 진행됩니다. ]",10);
				game->isteam=1;
			}
			else
			{
//				game_send_mes_t(game,"[ 팀 설정없이 일반게임으로 진행됩니다. ]",10);
			}
		}
        game->start_time = now;
	}
...
extern void game_send_mes_cb(t_connection * c, time_t now, t_timer_data str)
{
	if (!c)
	{
...
extern void game_send_mes(t_game *game,const char *str)
{
	int i;
	//if (game->status == game_status_open||game->status == game_status_full)
	{
		for (i=0; i<game->count; i++)
		{
			if (game->connections[i] != NULL)
			{
				message_send_text(game->connections[i],message_type_info,game->connections[i],str);
			}
		}
	}
}
#include "timer.h"
extern void game_send_mes_t(t_game *game,const char *str,int timec)
{
	int i;
	t_timer_data data;
	game_send_mes(game,str);
	//if (game->status == game_status_open||game->status == game_status_full)
	{
		for (i=0; i<game->count; i++)
		{
			...
			{
				data.p=xstrdup(str);
				timerlist_add_timer(game->connections[i],time(NULL)+timec,game_send_mes_cb,data);
				data.p=xstrdup("");
				timerlist_add_timer(game->connections[i],time(NULL)+timec,game_send_mes_cb,data);
				data.p=xstrdup(str);
				timerlist_add_timer(game->connections[i],time(NULL)+timec,game_send_mes_cb,data);
...
extern int game_add_player(t_game * game, char const * pass, int startver, t_connection * c)
{
    ...
    game_choose_host(game);
	{
		...
		if(game->status == game_status_open)
		{
	        for (i=0; i<game->count; i++)
	        {
	            if (game->connections[i] != NULL)
                {
                    sprintf(msgtemp,"[:: %s has JOINED ::]",conn_get_username(c));
                    message_send_text(game->connections[i],message_type_info,game->connections[i],msgtemp);
                }
            }
		}
	}
