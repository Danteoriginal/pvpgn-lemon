...
int mysql_query_low(char *que)
{
	return sql_mysql_query(que);
}
void mysql_escape_string_low(char *escape, const char *from, int len)
{
    sql_mysql_escape_string(escape, from, len);
}
...
static t_sql_field * sql_mysql_fetch_fields(t_sql_res *result)
{
    ...
    rfields = (char**)xmalloc(sizeof(t_sql_field) * (fieldno + 1));
    for(i = 0; i < fieldno; i++)
