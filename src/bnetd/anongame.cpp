...
static int _anongame_search_found(int queue)
{
	...
	{			/* trans support */
	     ...
	     * endpoing address of the bnet classs connection */
	    if (!w3ip)
 ...
 * bn_int_nset(). i hope it's worked on intel too %) */
	...
	eventlog(eventlog_level_error, __FUNCTION__, "[%d] got bad packet (classs %d)", conn_get_socket(c), packet_get_class(packet));
	return -1;
    }
