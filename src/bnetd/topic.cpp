    ...
    if (topiclist_head)
    {
        ...
        LIST_TRAVERSE(topiclist_head,curr)
        {
            ...
            if(strncasecmp(topic->channel_name,"ch-",3)==0
				...
				||strncasecmp(topic->channel_name,"ladderbang2",11)==0
				||strncasecmp(topic->channel_name,"Brood War-",10)==0
				)
                if(topic->topic)
...
int channel_set_topic
(char const * channel_name, char const * topic_text, int do_save)
{
    t_topic * topic;
    ...
    else
    {
        topiclist_add_topic(channel_name, topic_text,do_save);
            ...
            if(strncasecmp(channel_name,"ch-",3)==0
				||strncasecmp(channel_name,"op ",3)==0
				||strncasecmp(channel_name,"[    레더방     ]",17)==0
//				||strncasecmp(topic->channel_name,"ladderbang",10)==0
				||strncasecmp(channel_name,"[    레더, The Royal    ]",25)==0
//				||strncasecmp(topic->channel_name,"ladderbang2",11)==0
				||strncasecmp(channel_name,"Brood War-1",11)==0
				)
        if(topic_text)
        {
