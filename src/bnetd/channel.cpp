...
static chan_list_type channellist_head;
...
extern t_channel * channel_create(char const * fullname, char const * shortname, char const * clienttag, int permflag, int botflag, int operflag, int logflag, char const * country, char const * realmname, int maxmembers, int moderated, int clanflag, int autoname)
{
    ...
    if (permflag)
    {
        if ((channel = channellist_find_channel_by_fullname(fullname)))
        {
            ...
            {
                eventlog(eventlog_level_error,__FUNCTION__,"channel parameters do not match for \"%s\" and \"%s\"",fullname,channel->name.c_str());
                return NULL;
            }
        }
    }
...
//    channel = (t_channel*)xmalloc(sizeof(t_channel));
    ...
    eventlog(eventlog_level_debug,__FUNCTION__,"creating new_ channel \"%s\" shortname=%s%s%s clienttag=%s%s%s country=%s%s%s realm=%s%s%s",fullname,
	...
	channel->nameLower=channel->name;
	to_lower(channel->nameLower);
    ...
    else
	{
		...
		channel->shortnameLower=channel->shortname;
		to_lower(channel->shortnameLower);
	}
    ...
    if (logflag)
    {
        ...
        else
        {
            ...
            if (channel->shortname.length())
                fprintf(channel->log,"shortname=\"%s\"\n",channel->shortname.c_str());
            else
...
//    list_append_data(channellist_head,channel);
	channellist_head.push_back(channel);
...
extern int channel_destroy(t_channel * channel, t_elem ** curr)
{
	...
	channellist_head.get<tag_id>().erase(channel->id);
/*
	...
	*/
    ...
    eventlog(eventlog_level_info,__FUNCTION__,"destroying channel \"%s\"",channel->name.c_str());

    LIST_TRAVERSE(channel->banlist,ban)
    {
        char const * banned;

        if (!(banned = (const char*)elem_get_data(ban)))
    ...
    if(channel_get_flags(channel) & channel_flags_moderated) // moderated channel - only admins,OPs and voices may talk
    {
        if (type==message_type_talk || type==message_type_emote)
        {
            if (!((account_is_operator_or_admin(conn_get_account(me),channel_get_name(channel))) ||
					(channel_conn_has_tmpVOICE(channel,me)) || account_get_auth_voice(conn_get_account(me),channel_get_name(channel)) == 1))
            {
...
extern int channel_ban_user(t_channel * channel, char const * user)
{
    ...
    if (strcasecmp(channel->name.c_str(),CHANNEL_NAME_BANNED)==0 ||
            strcasecmp(channel->name.c_str(),CHANNEL_NAME_KICKED)==0)
        return -1;

    LIST_TRAVERSE_CONST(channel->banlist,curr)
    if (strcasecmp((const char*)elem_get_data(curr),user)==0)
        return 0;
...
extern int channel_unban_user(t_channel * channel, char const * user)
{
    ...
    LIST_TRAVERSE(channel->banlist,curr)
    {
        ...
        if (!(banned = (const char*)elem_get_data(curr)))
        {
            eventlog(eventlog_level_error,__FUNCTION__,"found NULL name in banlist");
            continue;
        }
...
extern int channel_check_banning(t_channel const * channel, t_connection const * user)
{
    ...
    LIST_TRAVERSE_CONST(channel->banlist,curr)
    if (conn_match(user,(const char*)elem_get_data(curr))==1)
        return 1;
...
extern char const * channel_get_shortname(t_channel const * channel)
{
    ...
    return channel->shortname.c_str();
}
...
///        LIST_TRAVERSE(channellist_head,curr)
		BEGIN(_channel,channellist_head)
        {
///            if (!(channel = (t_channel*)elem_get_data(curr)))
            ...
            if (channel->memberlist)
            {
                ...
                old_channel->shortname = (channel->shortname);
...
///        {
///            if (!(channel = (t_channel*)elem_get_data(curr)))
		BEGIN(_channel,channellist_old)
		{
			if(channel=*_channel)
            {
            ...
            while (memberlist)
            {
                ...
                conn_set_channel(member->connection, channel->shortname.c_str());
            }
        }
...
///        LIST_TRAVERSE(channellist_head,curr)
        BEGIN(_channel,channellist_head)
        {
            if (!(channel = *_channel)) /* should not happen */
            {
                eventlog(eventlog_level_error,__FUNCTION__,"channel list contains NULL item");
                continue;
            }
...
///        LIST_TRAVERSE(channellist_head,curr)
		string target(name);
		to_lower(target);
		BOOST_AUTO(it_res,channellist_head.get<tag_name>().find(target));
		BOOST_AUTO(it_err,channellist_head.get<tag_name>().end());
		if(it_res!=it_err)
		{
			return *it_res;
		}
 ...
 * Create a new_ channel if it is a permanent-type channel and all others
 * are full.
 */
...
//*/
	...
	{
		BOOST_AUTO(it_equal,channellist_head.get<tag_shortname>().equal_range(target));
		BOOST_AUTO(it_low,it_equal.first);
		BOOST_AUTO(it_high,it_equal.second);
		if(it_low!=it_high)
		{
			RANGE(_channel,it_low,it_high)
			{
				channel=*_channel;
				if (channel->maxmembers==-1 || channel->currmembers<channel->maxmembers)
				{
					eventlog(eventlog_level_debug,__FUNCTION__,"found permanent channel \"%s\" for \"%s\"",channel->name.c_str(),name);
					return channel;
				}
				maxchannel++;
			}
			...
			saveshortname = channel->shortname.c_str();
			...
			eventlog(eventlog_level_debug,__FUNCTION__,"created copy \"%s\" of channel \"%s\"",(channel)?(channel->name.c_str()):("<failed>"),name);
			return channel;
		}
		return 0;
	}
...
///                 * channel that would otherwise match. * /
                ...
                /* save off some info in case we need to create a new_ copy * /
     ...
     */
/*//
    if (foundperm) /* All the channels were full, create a new_ one * /
    {
        char * channelname;
...
{
	BOOST_AUTO(it_res,channellist_head.get<tag_id>().find(channelid));
	BOOST_AUTO(it_err,channellist_head.get<tag_id>().end());
	if(it_res!=it_err)
	{
		return *it_res;
	}
