...
#define EXIST_ACTION_RESUME     3 
    ...
    bn_byte_set(&packet->u.client_initconn.classs,CLIENT_INITCONN_CLASS_FILE);
    if (hexstrm)
    {
    ...
    if (hexstrm)
    {
	fprintf(hexstrm,"%d: send classs=%s[0x%02hx] type=%s[0x%04hx] length=%u\n",
		sd,
		packet_get_class_str(packet),(unsigned int)packet_get_class(packet),
		packet_get_type_str(packet,packet_dir_from_client),packet_get_type(packet),
		packet_get_size(packet));
	...
	if (hexstrm)
	{
	    fprintf(hexstrm,"%d: recv classs=%s[0x%02hx] type=%s[0x%04hx] length=%u\n",
		     sd,
