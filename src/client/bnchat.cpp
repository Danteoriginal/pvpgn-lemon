...
int str_print_term2(FILE * fp, char const * str, unsigned int len, int allow_nl)
{
    unsigned int i;
...
static int print_file(struct sockaddr_in * saddr, char const * filename, char const * progname, char const * clienttag)
{
    ...
    bn_byte_set(&ipacket->u.client_initconn.classs,CLIENT_INITCONN_CLASS_FILE);
	...
	str_print_term2(stdout,(const char*)packet_get_raw_data_const(fpacket,0),MAX_PACKET_SIZE,1);
    }
    filelen -= currsize;
    if (filelen)
    {
	...
	str_print_term2(stdout,(const char*)packet_get_raw_data_const(fpacket,0),filelen,1);
    }
    fflush(stdout);
...
void ansi_printf(t_client_state * client,int color, char const * fmt, ...)
{
    ...
    str_print_term2(stdout,buffer,0,1);
...
extern int main(int argc, char * argv[])
{
	...
	channellist = (const char**)xmalloc(sizeof(char const *)*1);
	for (i=0,chann_off=sizeof(t_server_channellist);
	    ...
	    channellist = (const char**)xrealloc(channellist,sizeof(char const *)*(i+2));
	...
	for (;;)
	{
		...
		case 1:
			...
			else
			{
			    ...
			    printf(" ");
			    str_print_term2(stdout,client.text,0,0);
			    printf("\n");
			    client.munged = 1;
			}
		...
		case 1:
			...
			
			{
				...
				printf(" ");
				str_print_term2(stdout,message,0,0);
				printf("\n");
				break;
