...
static t_packet * conn_create_packet(t_connection * c)
{
	...
	switch (c->classs) {
		...
		default:
			eventlog(eventlog_level_error,__FUNCTION__,"got bad connection classs %d",c->classs);
			return NULL;
	}
	if (!packet) {
...
static int conn_handle_packet(t_connection * c, t_packet * packet)
{
	...
	switch (c->classs) {
		...
		default:
			eventlog(eventlog_level_error,__FUNCTION__,"got bad connection classs %d (close connection)",c->classs);
			retval=-1;
			break;
	}
	return retval;
...
extern int d2cs_conn_destroy(t_connection * c, t_elem ** curr)
{
	...
	if (c->d2gs_id && c->classs==conn_class_d2gs) {
		d2gs_deactive(d2gslist_find_gs(c->d2gs_id),c);
	}
	if (c->classs==conn_class_bnetd) {
		s2s_destroy(c);
	}
	if (c->gamequeue) {
...
extern int d2cs_conn_set_class(t_connection * c, t_conn_class classs)
{
	ASSERT(c,-1);
	c->classs=classs;
	return 0;
				...
				unsigned int table_size)
{
	...
	ASSERT(table,-1);
		...
		eventlog(eventlog_level_error,__FUNCTION__,"got bad packet type %d (classs %d)",type,packet_get_class(packet));
		return -1;
	}
	if (size < table[type].size) {
		eventlog(eventlog_level_error,__FUNCTION__,"got bad packet size %d (type %d classs %d)",size,type,packet_get_class(packet));
		return -1;
	}
	if (!(c->state & table[type].state)) {
		eventlog(eventlog_level_error,__FUNCTION__,"connection %d state mismatch for packet type %d (classs %d)",c->sessionnum,
			type,packet_get_class(packet));
		return -1;
	}
	if (!table[type].handler) {
		eventlog(eventlog_level_error,__FUNCTION__,"missing handler for packet type %d (classs %d)",type,packet_get_class(packet));
		return -1;
	}
