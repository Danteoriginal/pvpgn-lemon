...
extern t_connection * s2s_create(char const * server, unsigned short def_port, t_conn_class classs)
{
...
extern int s2s_destroy(t_connection * c)
{
	...
	switch (d2cs_conn_get_class(c)) {
		case conn_class_bnetd:
			...
			break;
		default:
			eventlog(eventlog_level_error,__FUNCTION__,"got bad s2s connection classs %d",d2cs_conn_get_class(c));
			return -1;
	}
	return 0;
}
