...
extern int handle_bnetd_init(t_connection * c)
{
	...
	bn_byte_set(&packet->u.client_initconn.classs, CLIENT_INITCONN_CLASS_D2CS_BNETD);
	conn_push_outqueue(c,packet);
	packet_del_ref(packet);
	d2cs_conn_set_state(c,conn_state_connected);
	eventlog(eventlog_level_info,__FUNCTION__,"sent init classs packet to bnetd");
	return 0;
}
