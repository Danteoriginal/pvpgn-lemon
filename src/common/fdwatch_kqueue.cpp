...
/*	    eventlog(eventlog_level_trace, __FUNCTION__, "updating change event (read) fd on %d", ridx); */
	    ...
	    ridx = nochanges++;
	    _rridx[idx] = ridx;
/*	    eventlog(eventlog_level_trace, __FUNCTION__, "adding new_ change event (read) fd on %d", ridx); */
	}
	EV_SET(kqchanges + ridx, fdw_fd(cfd), EVFILT_READ, EV_ADD, 0, 0, (void*)((long long)idx));
    } 
...
/*	    eventlog(eventlog_level_trace, __FUNCTION__, "adding new_ change event (read) fd on %d", ridx); */
	...
	EV_SET(kqchanges + ridx, fdw_fd(cfd), EVFILT_READ, EV_DELETE, 0, 0, (void*)((long long)idx));
    }
...
/*	    eventlog(eventlog_level_trace, __FUNCTION__, "updating change event (write) fd on %d", ridx); */
	    ...
	    ridx = nochanges++;
	    _wridx[idx] = ridx;
/*	    eventlog(eventlog_level_trace, __FUNCTION__, "adding new_ change event (write) fd on %d", ridx); */
	}
	EV_SET(kqchanges + ridx, fdw_fd(cfd), EVFILT_WRITE, EV_ADD, 0, 0, (void*)((long long)idx));
    }
...
/*    eventlog(eventlog_level_trace, __FUNCTION__, "called fd: %d", fd); */
	...
	if (_rridx[idx] < nochanges)
	{
	    ...
	    oidx =(long long)(kqchanges[nochanges].udata);
...
static void _handle(t_fdwatch_fd *cfd,unsigned i)
{
...
/*	      eventlog(eventlog_level_trace, __FUNCTION__, "checking %d ident: %d read: %d write: %d", i, kqevents[i].ident, kqevents[i].filter & EVFILT_READ, kqevents[i].filter & EVFILT_WRITE); */
		cfd = fdw_fds + (long long)(kqevents[i].udata);
		//threadPool.schedule(boost::bind(_handle,cfd,i));
