...
extern t_packet * packet_create(t_packet_class classs)
{
    t_packet * temp;
    
    if (classs!=packet_class_init &&
	classs!=packet_class_bnet &&
	classs!=packet_class_file &&
	classs!=packet_class_udp &&
	classs!=packet_class_raw &&
	classs!=packet_class_d2game &&
        classs!=packet_class_d2cs &&
        classs!=packet_class_d2gs &&
	classs!=packet_class_d2cs_bnetd &&
	classs!=packet_class_w3route)
    {
	eventlog(eventlog_level_error,__FUNCTION__,"invalid packet classs %d",(int)classs);
        return NULL;
    }
...
extern t_packet_class packet_get_class(t_packet const * packet)
{
	...
	return packet_class_none;
    default:
	eventlog(eventlog_level_error,__FUNCTION__,"packet has invalid classs %d",(int)packet->classs);
	return packet_class_none;
    }
}
...
extern int packet_set_class(t_packet * packet, t_packet_class classs)
{
    if (!packet)
    {
	eventlog(eventlog_level_error,__FUNCTION__,"got NULL packet");
    ...
    if (packet->classs!=packet_class_raw)
    {
...
extern unsigned int packet_get_header_size(t_packet const * packet)
{
    ...
    default:
        eventlog(eventlog_level_error,__FUNCTION__,"packet has bad classs %d",(int)packet_get_class(packet));
        return MAX_PACKET_SIZE;
    }
}
