...
/* Added by Soar */
/* time limit for new_ member as newer(whom cannot be promoted) in clan, (hrs) */
...
#define  REPTO(i,nn,tt)      for(int i=(nn);(i)<=(tt);    (i)++)
#define RREPTO(i,nn,tt)      for(int i=(tt);(i)>=(nn);    (i)--)
#define  REPTOBY(i,nn,tt,by) for(int i=(nn);(i)<=(tt);(i)+=(by))

#define EACH(i,xx2) REP(i,(xx2).size()-1)
#define RANGE(it,itLow,itHigh)  for(BOOST_TYPEOF(itLow) it=itLow;it!=itHigh;++it)
#define BEGIN(it,x)  for(BOOST_AUTO(it,(x). begin()),_end_iterator_##it=(x). end();it!=_end_iterator_##it;++it)
#define RBEGIN(it,x) for(BOOST_AUTO(it,(x).rbegin()),_end_iterator_##it=(x).rend();it!=_end_iterator_##it;++it)
template<class _data, class _value>
void clear(_data &data, _value value)
{
	memset(data,value,sizeof(_data));
};
template<class _data, class _value>
bool haveit(_data &data,_value value)
{
